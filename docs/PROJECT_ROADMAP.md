...
| 8 | Final Polish + Handover | **DONE (partea fără domeniu) — 2026-09-21** (+ închideri: badge „UP” scos, favicon din semnul UVT, pagina `/confidentialitate` PROIECT — **oprită temporar din 2026-09-22** la cererea clientului, comutator `PRIVACY_ENABLED` (#13), e-mail final decis #7): `docs/MAINTENANCE.md`, docs aliniate la implementare, verificare completă pe build (format/tipuri/16 unit/46 e2e/fidelitate/audit). **Rămâne după activarea domeniului:** checklist-ul din `MAINTENANCE.md` §7 (verificări pe domeniu, Resend Verify, Search Console, export DNS, HSTS întărit, anulare Wix Premium) | — |
